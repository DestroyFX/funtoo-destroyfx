...
# 8.ble will be used for beta releases.
	SRC_URI="https://launchpad.net/ubuntu/+archive/primary/+files/fglrx-installer_8.780.orig.tar.gz"
	FOLDER_PREFIX=""
...
src_prepare() {
	...
	if kernel_is ge 2 6 36; then
		ewarn "Kernel >= 2.6.36 detected, More patching needed :)"
		epatch "${FILESDIR}"/ati-drivers-2.6.36.patch
	fi
        if kernel_is ge 2 6 36; then
                ewarn "Kernel >= 2.6.36 detected, Still More patching needed stupid dev :)"
                epatch "${FILESDIR}"/arch-2.6.36.patch
        else
                ewarn "Kernel < 2.6.36 detected, Still More patching needed stupid dev :)"
                epatch "${FILESDIR}"/arch-2.6.35_et_moins.patch
        fi
	epatch "${FILESDIR}"/ati-drivers-fix_compilation-bug-297322.patch

	if kernel_is ge 2 6 37; then
                ewarn "Kernel >= 2.6.37 detected, something changed since AMD worked on theses drivers... :)"
		epatch "${FILESDIR}"/sema_init.patch
	fi
